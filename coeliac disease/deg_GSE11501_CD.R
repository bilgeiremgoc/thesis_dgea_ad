...
gse1em <- getGEO("GSE11501", GSEMatrix = TRUE, AnnotGPL = TRUE)
...
group <- ifelse(grepl("control", metadata$characteristics_ch1, ignore.case = TRUE), "control", "CD")
group <- factor(group)
table(group)
...
deg <- subset(results, abs(logFC) > 0.1 & adj.P.Val < 0.05)
deg <- deg %>% filter(!is.na(deg$GeneSymbol))
head(deg)

write_xlsx(deg, "deg_GSE11501_CD.xlsx")

deg_filtered <- deg %>% filter(!is.na(deg$Gene.symbol))

deg_filtered <- deg %>%
  filter(adj.P.Val < 0.05 & abs(logFC) > 0.1)
nrow(deg_filtered)

up_genes <- deg_filtered %>% filter(logFC> 0.1)
down_genes <- deg_filtered %>% filter(logFC < -0.1)


write_xlsx(up_genes, "deg_GSE11501_CD_up_genes.xlsx")
write_xlsx(down_genes, "deg_GSE11501_CD_down_genes.xlsx")


# Volkan Plot
results$threshold <- as.factor(abs(results$logFC) > 0.1 & results$adj.P.Val < 0.05)
...
write_xlsx(as.data.frame(go_result_BP), "deg_GSE11501_CD_GO_BP_enrichment.xlsx")
...
write_xlsx(as.data.frame(go_result_MF), "deg_GSE11501_CD_GO_MF_enrichment.xlsx")
...
write_xlsx(as.data.frame(go_result_CC), "deg_GSE11501_CD_GO_CC_enrichment.xlsx")
...
write_xlsx(as.data.frame(kegg_enrich), "deg_GSE11501_CD_KEGG_enrichment.xlsx")
...
write_xlsx(as.data.frame(reactome_enrich), "deg_GSE11501_CD_SLE_reactome_enrichment.xlsx")
...
pheatmap(xcell_result, 
         main = "xCell Immune Infiltration - GSE11501",
         scale = "row",
         clustering_distance_rows = "euclidean",
         clustering_distance_cols = "euclidean")

write_xlsx(xcell_df, "deg_GSE11501_CD_SLE_xCell_Results.xlsx")
...
write_xlsx(stat_tests, "deg_GSE11501_CD_xcell_stat_tests.xlsx")
...
ggplot(long_xcell, aes(x = group, y = proportion, fill = group)) +
  ...
  theme_minimal() +
  labs(title = "Immune Cell Infiltration (xCell) - GSE11501") +
